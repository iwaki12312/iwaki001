...
**APIキー設定**:
1. `work/env.yaml.example` を `work/.env.yaml` にコピー
2. `work/.env.yaml` の `ELEVENLABS_API_KEY` に実際のAPIキーを設定
3. `work/.env.yaml` は `.gitignore` で除外されているため、Gitにコミットされない
   （環境変数 `ELEVENLABS_API_KEY` を直接設定しても動作する）
...
- **APIキー管理**: `work/.env.yaml` または環境変数 `ELEVENLABS_API_KEY` から自動読み込み。コードにAPIキーをハードコードしない
