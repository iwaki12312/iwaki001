...
- セットアップスクリプトはユーザーが手動で実行する形にする
- 外部ライブラリをinportしているので、使えそうなものがないか常に確認する（例: DOTween、CarToon）

### ゲーム考案時の方針
- シンプルにタップだけで遊べて、難しいルールは不要
- 0～3歳児むけなので、スコア等のUIは不要
- 文字やテキストも不要（読めないため）
- 視覚的にわかりやすく、ポップなデザイン
- 子供が好きなテーマを採用
...
## 新規ゲーム追加（テンプレート）
- テンプレートは [Assets/Games/_GameTemplate](Assets/Games/_GameTemplate)。新規ゲームはこのフォルダをコピーして `NN_ゲーム名` にリネームし、`Scenes/`・`Sprites/`・`Audios/` 等の仮アセットを一旦そのまま使って組み立てる。
